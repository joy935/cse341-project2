...
const getAllUsers = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json(error || "Some error occurred while fetching the users.");
  }
};
...
const getOneUser = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json(error || "Some error occurred while fetching the user.");
  }
};
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    const existingUser = await mongodb
      ...
      .db()
      .collection("users")
      .findOne({ _id: userId }); // check if the user exists
